...
char thread_state[][STATE_STR_LEN] = {
    "TS_INIT      ", "TS_READY     ", "TS_INTER     ", "TS_RUNNING   ",
    "TS_EXIT      ", "TS_WAITING   ", "TS_EXITING   "};
...
void arch_idle_ctx_init(struct thread_ctx *idle_ctx, void (*func)(void)) {
    ...
    /* X0-X30 all zero */
    for (i = 0; i < REG_NUM; i++) ec->reg[i] = 0;
...
void print_thread(struct thread *thread) {
    printk(
        "Thread %p\tType: %s\tState: %s\tCPU %d\tAFF %d\tBudget %d\tPrio: "
        "%d\t\n",
...
int sched_is_running(struct thread *target) {
    ...
    if (target->thread_ctx->state == TS_RUNNING) return 1;
    return 0;
}
 ...
 */
void sys_yield(void) {
	// reset budget so that the thread can be scheduled
    ((struct thread *)current_thread)->thread_ctx->sc->budget = 0;
    sched();
