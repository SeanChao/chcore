...
void thread_deinit(void *thread_ptr) {
    ...
    switch (thread->thread_ctx->state) {
        ...
        default:
            ...
            if (list_empty(&process->thread_list)) exit_process = true;

            destroy_thread_ctx(thread);
    }

    if (exit_process) process_exit(process);
}
...
static u64 load_binary(struct process *process, struct vmspace *vmspace,
    ...
    for (i = 0; i < elf->header.e_phnum; ++i) {
        ...
        if (elf->p_headers[i].p_type == PT_LOAD) {
            /*
             * Lab3: Your code here
             * prepare the arguments for the two following function calls:
             * pmo_init and vmspace_map_range. pmo_init allocates the demanded
             * size of physical memory for the PMO. vmspace_map_range maps the
             * pmo to a specific virtual memory address. You should get the size
             * of current segment and the virtual address to be mapped from elf
             * headers. HINT: we suggest you use the seg_sz and p_vaddr
             * variables for raw data extracted from the elf header and use
             * seg_map_sz for the page aligned segment size. Take care of the
             * page alignment when allocating and mapping physical memory.
             */
            ...
            kinfo("seg_sz: %lu, p_vaddr: %lx, seg_map_sz: %lu\n", seg_sz,
                  p_vaddr, seg_map_sz);
            ...
            /*
             * Lab3: Your code here
             * You should copy data from the elf into the physical memory in
             * pmo. The physical address of a pmo can be get from pmo->start.
             */

            paddr_t start_addr = pmo->start;
            vaddr_t v_start_addr = phys_to_virt(start_addr);
            kdebug(
                "(p)start_addr: %lx, v_start_addr: %lx, bin: %lx, off: %lx, "
                "filesz: %lx\n",
                start_addr, v_start_addr, bin, elf->p_headers[i].p_offset,
                elf->p_headers[i].p_filesz);
            kdebug("cpy %lx <- %lx\n", v_start_addr + (p_vaddr & OFFSET_MASK),
                   bin + elf->p_headers[i].p_offset);
            kdebug("pmo start=0x%lx\n", pmo->start);
            memcpy((char *)(v_start_addr + (p_vaddr & OFFSET_MASK)),
                   bin + elf->p_headers[i].p_offset,
                   elf->p_headers[i].p_filesz);
...
out_free_cap:
    for (--i; i >= 0; i--) {
        if (pmo_cap[i] != 0) cap_free(process, pmo_cap[i]);
    }
out_fail:
    return r;
}
...
int sys_set_affinity(u64 thread_cap, s32 aff) {
     ...
     * Finish the sys_set_affinity
     */
    if (INVALID_AFF(aff)) return -2;
    if (!thread || !thread->thread_ctx) return -3;
