...
static int read_write_pmo(u64 pmo_cap, u64 offset, u64 user_buf, u64 size,
    ...
    if (type == WRITE_PMO)
        ...
        r = copy_to_user((char *)user_buf,
                         (char *)phys_to_virt(pmo->start) + offset, size);
    else
...
int sys_map_pmo(u64 target_process_cap, u64 pmo_cap, u64 addr, u64 perm) {
    ...
    target_process = obj_get(current_process, target_process_cap, TYPE_PROCESS);
    if (!target_process) {
        r = -ECAPBILITY;
...
u64 sys_handle_brk(u64 addr) {
     ...
     * generates the mapping between  user heap's virtual address (already
     * stored in vmspace->user_current_heap) and the pmo you just created.
     *
     * HINT: For more details about how to create and initialize a pmo, check
     * function 'load_binary' for reference.
     *
     * If addr is larger than heap, the size of vmspace->heap_vmr and the size
     * of its coresponding pmo should be updated. Real physical memory
     * allocation are done in a lazy manner using pagefault handler later at the
     * first access time.
     *
     ...
     */
    obj_put(vmspace);
    kdebug("sys_handle_brk: retval=%lu\n", retval);
    return retval;
}
