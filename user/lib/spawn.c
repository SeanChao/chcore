...
int launch_process_with_pmos_caps(struct user_elf *user_elf,
    ...
    // Lab4 useless code, help avoid compile warning, you can delete it as you
    // wish
    transfer_caps[0] = 0;
...

    {
        ...
        for (i = 0; i < 2; ++i) {
            ...
            ret =
                usys_map_pmo(new_process_cap, user_elf->user_elf_seg[i].elf_pmo,
...

    {
        /**
         * Step A :
         * Transfer the capabilities (nr_caps) of current process to the
         * capabilities of child process
         */
...

    {
        ...
        stack_offset = MAIN_THREAD_STACK_SIZE - PAGE_SIZE;  // used in write_pmo
...

    {
         ...
         */
        ret = usys_write_pmo(main_stack_cap, stack_offset, init_env, PAGE_SIZE);
        if (ret != 0) {
...

    {
        ...
        main_thread_cap = usys_create_thread(new_process_cap, stack_va, pc,
...

    {
        ...
        if (child_process_cap) *child_process_cap = new_process_cap;
        if (child_main_thread_cap) *child_main_thread_cap = main_thread_cap;
    }
