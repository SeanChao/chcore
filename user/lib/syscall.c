...
u64 syscall(u64 sys_no, u64 arg0, u64 arg1, u64 arg2, u64 arg3, u64 arg4,
     ...
     * syscall returned, don't forget to move return value from x0 to the ret
     * variable of this function
     */
    asm("mov x8, %0\n\t" : : "r"(sys_no));
    // FIXME: $x8 is overwritten
    asm("mov x0, %0\n\t"
        "mov x1, %1\n\t"
        "mov x2, %2\n\t"
        "mov x3, %3\n\t"
...
int usys_unmap_pmo(u64 process_cap, u64 pmo_cap, u64 addr) {
    return syscall(SYS_unmap_pmo, process_cap, pmo_cap, addr, 0, 0, 0, 0, 0, 0);
}
...
u64 usys_register_server(u64 callback, u64 max_client, u64 vm_config_ptr) {
    return syscall(SYS_register_server, callback, max_client, vm_config_ptr, 0,
                   0, 0, 0, 0, 0);
}

u32 usys_register_client(u32 server_cap, u64 vm_config_ptr) {
    return syscall(SYS_register_client, server_cap, vm_config_ptr, 0, 0, 0, 0,
                   0, 0, 0);
}
...
int usys_cap_copy_to(u64 dest_process_cap, u64 src_slot_id) {
    return syscall(SYS_cap_copy_to, dest_process_cap, src_slot_id, 0, 0, 0, 0,
                   0, 0, 0);
}

int usys_cap_copy_from(u64 src_process_cap, u64 src_slot_id) {
    return syscall(SYS_cap_copy_from, src_process_cap, src_slot_id, 0, 0, 0, 0,
                   0, 0, 0);
}
...
int usys_write_pmo(u64 cap, u64 offset, void *buf, u64 size) {
    return syscall(SYS_write_pmo, cap, offset, (u64)buf, size, 0, 0, 0, 0, 0);
}

int usys_read_pmo(u64 cap, u64 offset, void *buf, u64 size) {
    return syscall(SYS_read_pmo, cap, offset, (u64)buf, size, 0, 0, 0, 0, 0);
}
